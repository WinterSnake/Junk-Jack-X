...
public sealed record class CharacterAppearance
{
	/* Constructor */
	public CharacterAppearance(bool isFemale, byte skinTone, byte hairStyle, HairColor hairColor)
	{
	...
	internal ushort Pack() => (ushort)(
		...
		(Convert.ToByte(this.IsFemale) << GENDER_SHIFT) |
		(this._HairStyle << STYLE_SHIFT) |
		((byte)this.HairColor << COLOR_SHIFT)
	);
	/* Static Methods */
	internal static CharacterAppearance Unpack(ushort @value)
	{
