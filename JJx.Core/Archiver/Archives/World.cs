...
public sealed class WorldArchive : IArchive
{
	...
	private void _LoadContainerForge()
	{
		...
		using (var chunk = this._Reader!.GetChunkStream(ArchiverChunkType.WorldForges))
		{
			var reader = new JJxReader(chunk);
			var count = reader.ReadInt32();
			this._World.Forges.EnsureCapacity(count);
			for (var i = 0; i < count; ++i)
				this._World.Forges.Add(reader.ReadObject<Forge>());
		}
		this._IsContainerForgeLoaded = true;
	}
	...
	private void _LoadContainerLab()
	{
		...
		using (var chunk = this._Reader!.GetChunkStream(ArchiverChunkType.WorldLabs))
		{
			var reader = new JJxReader(chunk);
			var count = reader.ReadInt32();
			this._World.Labs.EnsureCapacity(count);
			for (var i = 0; i < count; ++i)
				this._World.Labs.Add(reader.ReadObject<Lab>());
		}
		this._IsContainerLabLoaded = true;
	}
	private void _LoadContainerShelf()
	{
		...
		using (var chunk = this._Reader!.GetChunkStream(ArchiverChunkType.WorldShelves))
		{
			var reader = new JJxReader(chunk);
			var count = reader.ReadInt32();
			this._World.Shelves.EnsureCapacity(count);
			for (var i = 0; i < count; ++i)
				this._World.Shelves.Add(reader.ReadObject<Shelf>());
		}
		this._IsContainerShelfLoaded = true;
	}
	private void _LoadContainerPlant()
	{
		...
		using (var chunk = this._Reader!.GetChunkStream(ArchiverChunkType.WorldPlants))
		{
			var reader = new JJxReader(chunk);
			var count = reader.ReadInt32();
			this._World.Plants.EnsureCapacity(count);
			for (var i = 0; i < count; ++i)
				this._World.Plants.Add(reader.ReadObject<Plant>());
		}
		this._IsContainerPlantLoaded = true;
	}
	private void _LoadContainerFruit()
	{
		...
		using (var chunk = this._Reader!.GetChunkStream(ArchiverChunkType.WorldFruits))
		{
			var reader = new JJxReader(chunk);
			var count = reader.ReadInt32();
			this._World.Fruits.EnsureCapacity(count);
			for (var i = 0; i < count; ++i)
				this._World.Fruits.Add(reader.ReadObject<Fruit>());
		}
		this._IsContainerFruitLoaded = true;
	}
	...
	public void Write(IArchiveWriter writer)
	{
		...
		{
			var streamWriter = new JJxWriter(chunk);
			streamWriter.Write(this.Forges.Count);
			foreach (var forge in CollectionsMarshal.AsSpan(this.Forges))
				streamWriter.Write(forge);
		}
		// Container[Sign]
		...
		{
			var streamWriter = new JJxWriter(chunk);
			streamWriter.Write(this.Labs.Count);
			foreach (var lab in CollectionsMarshal.AsSpan(this.Labs))
				streamWriter.Write(lab);
		}
		// Container[Shelf]
		chunk = writer.WriteChunk(ArchiverChunkType.WorldShelves);
		{
			var streamWriter = new JJxWriter(chunk);
			streamWriter.Write(this.Shelves.Count);
			foreach (var shelf in CollectionsMarshal.AsSpan(this.Shelves))
				streamWriter.Write(shelf);
		}
		// Container[Plant]
		chunk = writer.WriteChunk(ArchiverChunkType.WorldPlants);
		{
			var streamWriter = new JJxWriter(chunk);
			streamWriter.Write(this.Plants.Count);
			foreach (var plant in CollectionsMarshal.AsSpan(this.Plants))
				streamWriter.Write(plant);
		}
		// Container[Fruit]
		chunk = writer.WriteChunk(ArchiverChunkType.WorldFruits);
		{
			var streamWriter = new JJxWriter(chunk);
			streamWriter.Write(this.Fruits.Count);
			foreach (var fruit in CollectionsMarshal.AsSpan(this.Fruits))
				streamWriter.Write(fruit);
		}
		// Container[Decay]
	...
	public List<Forge> Forges { get { this._LoadContainerForge(); return this._World.Forges; } }
	...
	public List<Shelf> Shelves { get { this._LoadContainerShelf(); return this._World.Shelves; } }
	private bool _IsContainerPlantLoaded = false;
	public List<Plant> Plants { get { this._LoadContainerPlant(); return this._World.Plants; } }
